...
const getPhotoUrl = async ({ fileName, uid, category, relatedEntityId }) => {
  try {
    console.log('[getPhotoUrl] Called with:', { fileName, uid, category, relatedEntityId });
    
    // GET THE STORAGE BUCKET
    const bucket = admin.storage().bucket();
    console.log('[getPhotoUrl] Storage bucket:', bucket.name);
    ...
    if (category) {
      ...
      switch (category) {
        ...
        case 'venue_images':
          ...
          if (!fileName) {
            const prefix = `users/${uid}/venue_images/${relatedEntityId}/`;
            console.log('[getPhotoUrl] Listing venue_images files with prefix:', prefix);
            const [files] = await bucket.getFiles({ prefix });
            console.log('[getPhotoUrl] Found', files.length, 'venue_images files');
            ...
            console.log('[getPhotoUrl] Returning venue_images file list:', fileList);
            return fileList;
          }
          filePath = `users/${uid}/venue_images/${relatedEntityId}/${fileName}`;
          break;
        ...
        case 'seatmap_images':
          ...
          if (!fileName) {
            ...
            console.log('[getPhotoUrl] Listing seatmap_images files with prefix:', prefix);
            const [files] = await bucket.getFiles({ prefix });
            console.log('[getPhotoUrl] Found', files.length, 'seatmap_images files');
            const fileList = files.map(file => {
              const fileName = file.name.split('/').pop();
              return {
                fileName,
            ...
            console.log('[getPhotoUrl] Returning seatmap_images file list:', fileList);
            return fileList;
          }
          filePath = `users/${uid}/seatmap_images/${relatedEntityId}/${fileName}`;
          break;
    ...
    // GET DOWNLOAD URL (PUBLIC URL)
    const downloadURL = `https://storage.googleapis.com/${bucket.name}/${encodeURI(filePath)}`;
    console.log('[getPhotoUrl] Generated download URL for filePath:', filePath);
    console.log('[getPhotoUrl] Download URL:', downloadURL);

    // RETURN THE DOWNLOAD URL
    return downloadURL;
  } catch (error) {
    console.error('[getPhotoUrl] Error getting photo URL:', error);
    throw error; // Re-throw so callers can handle it
  }
};
